...
import { readState, writeState, readGlobalConfig, log, warn, error, ensureDir, writeFileSafe, readFileSafe, beatFileName } from "./utils.js";
...
function backupBeforeWrite(root, filePath, chapterNum) {
...
function assembleChapter(root, chapterNum, cfg) {
...
export async function writeSingleBeat(root, cfg, chapterNum, beatIndex, extraReq, previousText) {
    const chapterCfg = cfg.chapters.find(c => c.chapter === chapterNum);
    if (!chapterCfg)
        error(`novel.toml 中没有第 ${chapterNum} 章配置`);
    const beat = chapterCfg.beats[beatIndex];
    if (!beat)
        error(`第 ${chapterNum} 章没有第 ${beatIndex + 1} 段`);
