...
import type { NovelConfig, GlobalConfig } from "./types.js";
import type { StructuredContext } from "./context.js";
import { log, warn } from "./utils.js";
import * as fs from "fs";

export interface ModelRequest {
  context?: StructuredContext;  // 结构化上下文（写作用）
  rawPrompt?: string;          // 原始 prompt（摘要等简单任务用）
  endpoint: string;
  model: string;
  apiKey: string;
  temperature?: number;
...
function buildMessages(ctx: StructuredContext): ChatMessage[] {
...
function buildRawMessages(prompt: string): ChatMessage[] {
...
function isLikelyTruncated(text: string, completionTokens: number, maxTokens: number): boolean {
  const trimmed = text.trim();
  if (trimmed.length < 20) return true;
  // 模型用满 token 配额，极可能截断
  if (completionTokens >= maxTokens) return true;
...
export function resolveModelConfig(
  cfg: NovelConfig,
  global: GlobalConfig
): { endpoint: string; model: string; apiKey: string; fallbackModel?: string } {
  return {
