...
import type { NovelConfig, BeatConfig } from "./types.js";
import {
  readBeatFiles, readSummary, readAssembled, readFileSafe, warn, error,
} from "./utils.js";
...
export interface ContextInput {
  ...
  cfg: NovelConfig;
  chapterNum: number;
  beatIndex: number;       // 0-based index within chapter
  beat: BeatConfig;
  extraReq?: string;       // --req from CLI
  previousText?: string;   // redo 时注入的原文，供模型参考修改
}

export interface StructuredContext {
  system: string;
  contextBlocks: string[]; // 每个 block 放入一个 assistant 块
  task: string;            // 放入 user 块
}

export function buildContext(input: ContextInput): StructuredContext {
  ...
  if (summaryStart >= 1) {
    ...
    if (summaryLines.length > 0) {
      ...
      if (hasMissing) error("部分章节摘要缺失，运行 `novel summarize chN` 补充");
    }
  }
  ...
  for (let c = nextChapter; c <= nextChapter + 1; c++) {
    if (c > cfg.chapters.length) break;
    const assembled = readAssembled(path.join(root, "chapters", chDirName(c)));
    if (assembled) {
...
function styleBlock(cfg: NovelConfig): string {
  return `【文体设定】
风格基调: ${cfg.style.tone}
...
function characterBlock(cfg: NovelConfig): string {
  const lines = cfg.characters.map(c =>
    `${c.name}（${c.role}${c.age ? `, ${c.age}岁` : ""}）: ${c.desc}`
  );
  return `【角色】\n${lines.join("\n")}`;
}

function chDirName(chapterNum: number): string {
