...
import {
  projectDir, readNovelConfig, readState, writeState,
  readGlobalConfig, ensureDir, writeFileSafe, readFileSafe,
  readBeatFiles, readAssembled, readSummary, log, warn, error,
} from "./utils.js";
import { writeSingleBeat } from "./write.js";
import { callModel, resolveModelConfig } from "./model.js";
import type { NovelConfig } from "./types.js";
...
async function cmdInit() {
  ...
  if (configPath) {
    ...
    if (!raw) error(`配置文件不存在: ${configPath}`);
    const cfg = JSON.parse(raw) as NovelConfig;
    ensureDir(targetDir);
...
async function cmdWrite() {
  ...
  if (!chapterCfg) error(`novel.json 中无第 ${chapterNum} 章配置`);
...
async function cmdRedo() {
  ...
  if (beatIndex === undefined) error("redo 需指定段号，如 novel redo ch3 beat2");
...
async function cmdUndo() {
  ...
  const files = fs.readdirSync(backupDir).filter((f: string) => f.endsWith(".md"));

  if (files.length === 0) error("无可用备份");
  ...
  for (const ch of cfg.chapters) {
    ...
    if (beatFiles.length === 0) continue;
...
async function cmdSummarize() {
  ...
  if (!chMatch) error("请指定章节，如 novel summarize ch3");
  ...
  if (textIdx > -1 && cmdArgs[textIdx + 1]) {
    ...
    writeFileSafe(
      path.join(root, "context", "summaries", `ch${String(chNum).padStart(2, "0")}.md`),
      text
    );
  ...
  if (!chapterText) error(`Ch.${chNum} 未找到 assembled.md。请先完成该章写作`);
...
async function cmdPreview() {
  ...
  const chMatch = cmdArgs[0]?.match(/^ch(\d+)$/i);
  if (!chMatch) error("请指定章节，如 novel preview ch3");
...
async function cmdBuild() {
  ...
  if (genre) lines.push(`║${centerPad(genre, 38)}║`);
  lines.push("╚══════════════════════════════════════╝");
  lines.push("");
...
/** Center-pad a string to given width */
function centerPad(s: string, width: number): string {
  const len = [...s].length; // count CJK chars properly
...
function promptSync(msg: string): string {
  ...
  while (true) {
    ...
    if (n <= 0 || chunk[0] === 0x0a) break;
    if (chunk[0] !== 0x0d) bufs.push(Buffer.from([chunk[0]]));
  }
